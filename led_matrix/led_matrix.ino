char msg[] = " PSK CREATIONS ";  //Change the text here.
int scrollspeed = 30;                          //Set the scroll speed ( lower=faster)
  ...
  for (int charIndex = 0; charIndex < (sizeof(msg) - 1); charIndex++) {
    ...
    for (int col = 0; col < 6; col++) {
      for (int row = 0; row < 8; row++) {
        ...
        bool isOn = 1;
        if (col < 5) isOn = bitRead(alphabets[alphabetIndex][col], 7 - row) == 0;
